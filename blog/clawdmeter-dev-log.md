...
## 功能四：Idle Nudge——别闲着，回去写代码

长时间不用 Claude,桌宠会「催你」。

当 session 用量百分比连续 5 分钟没有变化（即你一直没在用 Claude）,
屏幕自动弹出一个全屏覆盖层：左边是经典鼠标指针手型，朝右指向一只
正在敲代码的 Clawd 像素动画——意思很明确：「点我，回去干活。」

![Idle Nudge 覆盖层](images/nudge.png)

几个实现要点：

- **手型图标**是经典鼠标 cursor hand 的 90° 旋转版，16×11 像素网格
  以 10 倍放大渲染到 LVGL canvas 上，白色填充、灰色描边，在 AMOLED
  黑底上非常清晰。
- **Clawd 动画**复用了 splash 模块的 `work coding` 动画，用新增的
  `splash_render_scaled()` 接口以 12 倍放大渲染到独立的 240×240
  canvas，不干扰主 splash 的全屏渲染。
- **手型左右晃动**用 LVGL 内置动画引擎做 ±10px 的水平往复，700ms
  一个周期，制造「戳一戳」的视觉暗示。
- **点击即跳转**——轻触覆盖层，通过 BLE 向主机发送 `0x20` 指令,
  守护进程收到后调用 `open -a Claude`（macOS）或 `xdg-open`（Linux）
  打开 Claude 应用，覆盖层随即消失。
- **循环触发**——覆盖层被点掉后重新计时，再次空闲够久会再弹出,
  直到 session 数据真正发生变化才停止催促。

> 为了录演示视频，把触发阈值从 5 分钟临时改成了 5 秒。
